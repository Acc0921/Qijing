...
- `SceneTaskEvent` 已按 task id 串联命中、预检、快照、写入/预演、读回验证、恢复和恢复不完整；总览与场景页通过可观察事件账本实时刷新。
- 正式场景事务在首次写入前同步持久化完整恢复计划；逐项记录 `PENDING/WRITE_STARTED/APPLIED/RESTORED`，服务重启优先恢复并保持自动化停止。

### 9.3 仍需实现

- 预演报告的设备能力摘要、持久化指纹和跨页面回看。
- 真机 Root/Shizuku 写入、读回、离场恢复和授权撤销验收。
- 更细的恢复账本详情页，包括每项原值、目标值、实际恢复读回和人工重试入口；当前任务轨迹已能显示能力级恢复进度和恢复不完整状态。
