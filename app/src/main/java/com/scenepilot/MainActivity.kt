...
private fun SceneServiceControl() {
    val context = LocalContext.current
    val source = remember { com.scenepilot.core.scene.UsageStatsForegroundAppSource(context) }
    val lifecycleOwner = LocalLifecycleOwner.current
    var access by remember { mutableStateOf(source.accessState()) }
    var running by remember { mutableStateOf(false) }
    DisposableEffect(lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event -> if (event == Lifecycle.Event.ON_RESUME) access = source.accessState() }
    ...
    Card(modifier = Modifier.fillMaxWidth().padding(top = 12.dp)) { Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Text(if (running) "轮询运行中（dry-run）" else "服务未启动", color = MaterialTheme.colorScheme.primary)
    } }
}
