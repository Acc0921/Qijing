...
- Android/Kotlin，UI 使用 Jetpack Compose + Material 3 + Navigation Compose，并通过就绪状态、原生列表、场景链路、任务轨迹和会话图表建立独立操作语言。
...
当前开发环境使用 JDK 17、Android SDK 35 与 Gradle 8.11.1；提交前运行 JVM 测试、AndroidTest 设备回归、lint 和 debug/release APK 构建。场景事务已具备结构化实时轨迹和正式跨进程恢复 journal；真实 Root/Shizuku 写入仍以逐项真机验收记录为发布门槛。若进入 Google Play，`specialUse` 前台服务与全包可见性必须另行完成政策申报。
